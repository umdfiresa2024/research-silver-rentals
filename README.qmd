...
<<<<<<< HEAD
Answer: $$ZORI_{zm} = \beta_1 silver_z + \beta_2 + \epsilon_{zm}$$
...
>>>>>>> d011a6ee49435fa1ba7e3103cb6a571d6b033df4

## Question 9: What is the impact of the treatment effect once fixed effects are included?

```{r}
<<<<<<< HEAD
model2 <- lm(ZORI ~ silver + post + silver:post + dist + as.factor(month) + as.factor(year) + as.factor(City) + as.factor(ZIPCODE), data=df)
model3 <- lm(log(ZORI) ~ silver + post + silver:post + dist + as.factor(month) + as.factor(year) + as.factor(City) + as.factor(ZIPCODE), data=df)
model4 <- lm(ZORI ~ silver + post + silver:post + dist + exptime + as.factor(month) + as.factor(year) + as.factor(City) + as.factor(ZIPCODE), data=df)
# summary(model2)
summary(model4)
```
